...
% GET
To recieve the value of a index in the grid, the frontend has to send a GET request and attach the index value to the query string. For example, to get the value of the second card in the grid, the URL has to look like the \path{http://localhost/memory/grid?index=2}.
% Response
...
% Modes
In addition, it is possible to set the operation mode, which decides if the grid is generated randomly or deterministic. This property is set as a init-param in the \path{web.xml}, as illustrated by FIG BC. Then, the Memory-Grid-Servlet can read this value using \texttt{getInitParameter("mode")}.
