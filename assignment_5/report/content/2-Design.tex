\section{Design}\label{sec:02_impl}
% What
...
To receive a list of all available accommodations, the \textit{AccommodationResultServlet} delegates the request details to the \textit{AccommodationService} (introduced in \Sec{sec:02_impl_beans_acc}) to call the \texttt{getAvailableAccommodations} method of the Bean.
% ReservationService
The total prices are calculated by delegating the details to the \texttt{calculateApartmentReservationPrice} and \texttt{calculateHotelReservationPrice} methods of the \textit{ReservationService} (introduced in \Sec{sec:02_impl_beans_reservation}) respectively.

\subsubsection{ReservationSummaryServlet}\label{sec:02_impl_web_reservationsummary}
% What
...
% How
There, the user has to provide the first name, and last name, same as given at the \textit{ReservationSummaryServlet} (introduced in \Sec{sec:02_impl_web_reservationsummary}), shown in FIG XY. After submitting, the \textit{ReservationListServlet} sends a POST request to itself to present all reservation made by the given user, shown in FIG AB.
