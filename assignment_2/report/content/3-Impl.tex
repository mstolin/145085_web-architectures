...
% MVC
As mentioned in \Sec{sec:01_intro}, the MVC architecture is used to implement this application. \Fig{fig:03_impl_structure} present the project folder structure of the MVC implementation.
% What is what
...
% Why
As being mentioned in \Sec{subsec:02_design_datastorage}, a custom store is needed to save models introduced in \Sec{subsec:03_impl_models}.
...
% Singleton pattern
To make the same set of Users, and Rooms available for each single servlet instance, the singleton pattern is used for the stores. This ensures, that all servlets work with the same set of \texttt{Users} and \texttt{Rooms}. An example of the singleton usage is shown in \Lst{lst:03_impl_objstores_singleton}.
% Singleton example
...
% Logout
To logout, the \texttt{AuthLogoutServlet} exist. If a \texttt{GET} request is made from an active user, it invalidates the \texttt{HTTPSession} and redirects the user to the \textit{Login-Page}.
...
% Servlet
The \texttt{AdminServlet}, validates the the \texttt{POST} request attributes and checks if the user already exist. If the user does not exist, the user will be add via the \texttt{addUser} method of the \texttt{UserStore}. As mentioned in \Sec{subsec:03_impl_objstores}, the \texttt{UserStore} will write the user to the \texttt{users.txt} file.
% Check credentials
...
% The banner
As mentioned in \Sec{subsec:02_design_routes}, a \textit{Banner} is included in each page.
% No servlet
...
% Admin
Additionally, as mention in \Sec{subsubsec:02_design_routes_admin}, only the \textit{admin} user is allowed to access the \textit{Admin-Page}. Therefore, an additional filter is needed which ensures this behaviour.
% Which filter
